...
def calculate_distance_to_camera(face_width_pixels, known_width=16.0):
    """
    Calculate approximate distance from face to camera
    known_width: Average human face width in cm
    focal_length: Approximate focal length (can be calibrated for better accuracy)
    """
    # Approximate focal length for typical webcam (can be calibrated for better accuracy)
    focal_length = 600  
    
    # Convert face coordinates to actual width in pixels
    face_width_in_frame = face_width_pixels
...
def main():
    ...
    while True:
        ...
        for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
            ...
            face_width = x2 - x1
            physical_distance = calculate_distance_to_camera(face_width)
            
            # Draw rectangles
            cv2.rectangle(img, (x1, y1), (x2, y2), (128, 0, 128), 2)
            cv2.rectangle(img, (x1, y2 - 35), (x2, y2), (128, 0, 128), cv2.FILLED)
            
            # Display name, confidence score, and physical distance
            confidence_text = f"{nama} ({faceDis[matchIndex]:.2f})"
            distance_text = f"{physical_distance:.0f} cm"
            
            # Display confidence on top
            cv2.putText(img, confidence_text, (x1 + 6, y2 - 20), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 1)
            # Display physical distance below
            cv2.putText(img, distance_text, (x1 + 6, y2 - 6), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 1)
